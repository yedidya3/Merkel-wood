...
class sparseMerkleTree:
    def __init__(self):
        ...
        self.root = NodeMarkel(self.hashBlankVertices[255],None,None,None,4)#כאן זה הרמה 255

    def MarkLeaf(self ,digitStr):
        digitArray = list(digitStr)
        if(len(digitArray) != 255):
            print("not enough digit")
            #return
        if((digitArray.count('0')+digitArray.count('1')) != len(digitArray)):
            print(len(digitArray))
            return
    ...
    def createProofOfInclusionRecurse(self ,node ,digitArray,arrayOfInclusion ):
        if(node.level <= 0):
            ...
            if(digitArray[0] == '0'):
                ...
                if(node.left == None):
                    for i in range(node.level-2, -1, -1):
                        arrayOfInclusion.append(self.hashBlankVertices[i])
                    return
                else:
    ...
    def checkProofSMTRecursion(self ,node , arrayDigest ,classification ,arrayProof):
        if(node.level == 0):
            ...
            if(arrayDigest[0] == '0'):
                if(node.right == None):
                    if(arrayProof[0] != self.hashBlankVertices[node.level-1]):
                        return False
                else:
                    if(arrayProof[0] != node.right.data):
                        return False
                del arrayDigest[0]
                del arrayProof[0]
                if(node.left == None):
                    ...
                    for i in range(node.level-1):
                        if(arrayProof[i] != self.hashBlankVertices[i]):
                            return False
                    return True
                return self.checkProofSMTRecursion(node.left , arrayDigest ,classification ,arrayProof)
            ...
            if(arrayDigest[0] == '1'):
                if(node.left == None):
                    ...
                    if(arrayProof[0] != node.left.data):
                        return False
                del arrayDigest[0]
                del arrayProof[0]
                if(node.right == None):
                    ...
                    for i in range(node.level-1):
                        if(arrayProof[i] != self.hashBlankVertices[i]):
                            return False
                    return True
                return self.checkProofSMTRecursion(node.right , arrayDigest ,classification ,arrayProof)   
    ...
    #\nnwIDAQAB \x05]\'z8E\xb2\xb8\x1d\t\xb3K\x03\x7f(\x91Yz\xaaO\xf8=\'~\x9bn\x00u\x91\xb5\xaf\xa9h\xcc\xd0B\xf5\xa7\xed\xcc\x0fi\x96:\x05"\xc5%\x02\xdd\xd8\x1a\xd7\x88\xc2\x88\x02\x0f}!\xf5\xda\xa3^\x14\x16\xf9\xab\x9d\x1eR\x1a\x0b\t\xad\xea|3E\xd8\x9b\x0c\x02\xfe\xc8\x8b\xd9S\xed\x14"\xe7\xf7\xa7|\x94\xe8\xda\x81\x08\xf8@\xa2/s7\xf2o3\x92\x99\xe3\xc5\xb0K\x13\x931\xa8\xaa\xff\xf4\xc5\x7f\xff3\xb4\xb4\xddYXC&\xf3`\xe5\xfc\x9a\xc2\xd1\xe2\'\xd7\xe5\xfa\t\xee\x148\xb1Pa":\xc1t\xca\x88\xce\xbeV\x80\xde7%\x1eq\xe90\x99u\xe7F_v@\x8bUi\xbbg\xca\x82\xa2V\xa1n><\x0e\x96\xc6\x01m^\xa7a\xc7\t\x129a7\x95A\xe2\xb1S\x1b@\xb1\xe4\xd2\xe1LH\xdf=j\xe7\x83+\x1a4\xdb\x9ex=i\xff\x1e\xa5\xe9\xd5\xff\x18\xb2\x16g\xa1J\x14c\x96t\xe9\xdb\x15h\x19\x05\xb4\xca\xe8\xea\x9e 789a758b0ce412b1ded74d9e8482de93c9e1c1e0bd2d8a2a8c83cbc7ec7e0dca
        ...
        if(x[0] == '11'):
            try:
                ...
                print() 
# 11 0000 0 d133ce81cee147cf7f9473af9fb106a25d7dc3c34e916ba45ad2547ab79fc36f 0 5feceb66ffc86f38d952786c6d696c79c2dbc239dd4e91b46729d73a27fb57e9 5feceb66ffc86f38d952786c6d696c79c2dbc239dd4e91b46729d73a27fb57e9 1485b3ac3d7f6a5b7b7334717742ec6741c827083f4a609a5b944c3e08c034e5                      
